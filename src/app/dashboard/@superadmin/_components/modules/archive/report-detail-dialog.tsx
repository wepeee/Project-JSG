...
export function ReportDetailDialog({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[90vh] flex-col gap-0 overflow-hidden p-0 outline-none sm:max-w-6xl">
        <DialogHeader className="bg-background/95 z-10 shrink-0 border-b px-6 py-4 backdrop-blur">
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-4 md:grid-cols-4 lg:grid-cols-5">
            ...
            <div className="col-span-2 space-y-1">
              ...
              <div
                ...
                title={report.proses.pro.productName}
              >
                ...
                {isPaper && (
                  <span>
                    ...
                    {(() => {
                      ...
                      const totalOutput =
                        Number(report.qtyPassOn || 0) +
                        Number(report.qtyHold || 0) +
                        Number(report.qtyWip || 0);
            ...
            <div className="space-y-1">
              ...
              <div
                ...
                title={report.notes || "Tidak ada catatan"}
              >
                {report.notes || "-"}
              </div>
            </div>
              ...
              <div className="col-span-3 mt-2 flex items-center justify-between rounded bg-slate-100 px-3 py-1 md:col-span-1 md:mt-0 md:flex-col md:items-start md:justify-center md:border-l-0 dark:bg-slate-800">
                ...
                <div className="font-mono text-xl font-black text-slate-700 dark:text-slate-200">
                  {(() => {
                    ...
                    if (category !== "PAPER") {
                      const pw = Number(
                        (report.metaData as any)?.productWeight,
                      );
                      if (pw > 0)
        ...
        <div className="scrollbar-thin bg-muted/5 overflow-y-auto p-6">
          {/* 3. Rejects & Downtime */}
                      ...
                      if (category === "PAPER") {
                        ...
                        INJECTION_REJECT_BB.forEach((col) => {
                          ...
                          if (col === "Gilingan / Purge")
                            val = report.materialPurgeQty;
                          else if (col === "PT / Runner")
                            val = report.materialRunnerQty;
                          else
                            val =
                        ...
                        INJECTION_REJECT_PROD.forEach((col) => {
                          const val =
                            breakdown[`REJECT:PRODUK:${col}`] || breakdown[col];
                          if (val > 0) items.push({ label: col, value: val });
                        });
                      }
                      ...
                      if (items.length === 0) {
                        return (
                          <TableRow>
                            <TableCell
                              colSpan={2}
                              className="text-muted-foreground py-8 text-center text-xs opacity-50"
                            >
                ...
                <Table>
                  ...
                  <TableBody>
                    {(() => {
                      ...
                      const items = keys
                        .map((k) => ({ label: k, value: Number(breakdown[k]) }))
                        .filter((i) => i.value > 0);
